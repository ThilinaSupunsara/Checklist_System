<x-app-layout>
    <x-slot name="header">
        <div class="page-header-container">
            <div>
                <h2 class="page-header-title">{{ __('Add New Property') }}</h2>
                <p class="page-header-subtitle">Fill out the form below to add a new property to your account.</p>
            </div>
        </div>
    </x-slot>
            ...
            <form method="POST" action="{{ route('owner.properties.store') }}">
                ...
                <div class="form-grid">
                    <div class="form-group form-group--full-width">
                        ...
                        @error('name')
                            <p class="form-error-message"><span>{{ $message }}</span></p>
                        @enderror
                    </div>

                    <div class="form-group">
                        <label for="beds" class="form-label">{{ __('Number of Beds') }}</label>
                        <input id="beds" class="form-input @error('beds') is-invalid @enderror" type="number" name="beds" value="{{ old('beds') }}" required />
                        @error('beds')
                            <p class="form-error-message"><span>{{ $message }}</span></p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input id="baths" class="form-input @error('baths') is-invalid @enderror" type="number" name="baths" value="{{ old('baths') }}" required />
                        @error('baths')
                            <p class="form-error-message"><span>{{ $message }}</span></p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input id="latitude" class="form-input @error('latitude') is-invalid @enderror" type="text" name="latitude" value="{{ old('latitude') }}" placeholder="e.g., 40.7128" />
                        @error('latitude')
                            <p class="form-error-message"><span>{{ $message }}</span></p>
                        @enderror
                    </div>
                    ...
                    <div class="form-group">
                        ...
                        <input id="longitude" class="form-input @error('longitude') is-invalid @enderror" type="text" name="longitude" value="{{ old('longitude') }}" placeholder="e.g., -74.0060" />
                        @error('longitude')
                            <p class="form-error-message"><span>{{ $message }}</span></p>
                        @enderror
                    </div>
